...
export function RevenueBookingsChart({ series, granularity, currency }: RevenueBookingsChartProps) {
  ...
  return (
    <section className="rounded-xl border border-neutral-dark bg-neutral-light p-4 shadow-sm">
      <div className="mb-4">
        <h2 className="text-lg font-semibold text-primary-dark">Ingresos realizados y reservas</h2>
        <p className="text-sm text-primary-light">Ingresos realizados corresponden solo a reservas completadas.</p>
      </div>
      <div className="h-72">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 8, right: 16, left: 0, bottom: 8 }}>
            ...
            <YAxis yAxisId="revenue" tickFormatter={(value) => formatCurrency(Number(value), currency)} width={88} />
            <YAxis yAxisId="count" orientation="right" tickFormatter={(value) => formatInteger(Number(value))} width={40} />
            <Tooltip
